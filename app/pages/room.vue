    ...
    <div class="chat-area" ref="messagesContainer">
      ...
      <template v-for="msg in messages" :key="msg.id">
        ...
        <div 
          v-else 
          class="msg-row" 
          :class="{ 'msg-me': isMe(msg.author), 'msg-pending': msg.pending }" 
        >
          ...
          <div class="bubble">
            ...
            <div class="msg-footer">
              ...
              <span v-if="msg.pending" class="icon-pending" title="En attente de connexion...">🕒</span>
            </div>
          </div>
        </div>
...
.app-container {
  ...
  background-color: #f0f2f5; font-family: 'Segoe UI', Roboto, sans-serif; overflow: hidden;
}

/* --- HEADER --- */
.top-bar {
  flex-shrink: 0; height: 64px; background: rgba(255, 255, 255, 0.95);
...
.back-btn, .gallery-btn {
  background: none; border: none; cursor: pointer; border-radius: 50%; padding: 8px; transition: 0.2s;
}
.back-btn:active, .gallery-btn:hover { background: #eee; }
.back-btn { font-size: 1.2rem; } .gallery-btn { font-size: 1.5rem; }

.room-avatar {
  width: 40px; height: 40px; background: linear-gradient(135deg, #667eea, #764ba2);
  color: white; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: bold;
}
.room-text { display: flex; flex-direction: column; }
.room-name { margin: 0; font-size: 1rem; font-weight: 700; text-transform: capitalize; }
.status { font-size: 0.75rem; color: #4caf50; font-weight: 500; }

/* --- CHAT AREA --- */
.chat-area {
  flex-grow: 1; overflow-y: auto; padding: 20px;
  display: flex; flex-direction: column; gap: 8px; background-color: #f9f9f9;
}
.empty-placeholder { text-align: center; margin-top: 40px; color: #888; }
...
.msg-row.msg-me { align-self: flex-end; flex-direction: row-reverse; }

/* === STYLE PENDING (HORS LIGNE) === */
.msg-pending .bubble {
  opacity: 0.7;
  border: 1px dashed #ccc;
  transition: opacity 0.3s;
}
.icon-pending { font-size: 0.8rem; margin-left: 5px; cursor: help; }
...
.bubble {
  ...
  box-shadow: 0 1px 2px rgba(0,0,0,0.05); background: white; color: #1c1e21;
}
.msg-me .bubble { background: linear-gradient(135deg, #667eea, #764ba2); color: white; border-bottom-right-radius: 4px; }
.msg-row:not(.msg-me) .bubble { border-bottom-left-radius: 4px; }

.sender-name { display: block; font-size: 0.7rem; color: #666; margin-bottom: 4px; font-weight: 600; }
.msg-me .sender-name { display: none; }

.media-content img { border-radius: 12px; margin-bottom: 5px; max-width: 100%; display: block; }
.text-content { margin: 0; font-size: 0.95rem; line-height: 1.4; word-break: break-word; }

.msg-footer { display: flex; justify-content: flex-end; align-items: center; margin-top: 4px; }
.time-tick { font-size: 0.65rem; opacity: 0.7; }
...
/* --- ACTION BAR --- */
.action-bar {
  flex-shrink: 0; background: white; padding: 10px 16px; display: flex; align-items: center; gap: 10px;
  border-top: 1px solid rgba(0,0,0,0.05);
}
.hidden-input { display: none; }
.action-btn { border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; }
.action-btn.secondary { background: transparent; font-size: 1.4rem; color: #65676b; padding: 6px; }
.input-wrapper { flex-grow: 1; background: #f0f2f5; border-radius: 20px; padding: 8px 15px; }
.input-wrapper input { width: 100%; border: none; background: transparent; outline: none; }
.send-btn {
  width: 40px; height: 40px; border-radius: 50%; background: #667eea; color: white;
  font-size: 1.1rem; box-shadow: 0 2px 5px rgba(102, 126, 234, 0.4);
}
...
.overlay-controls {
  ...
  background: linear-gradient(to top, rgba(0,0,0,0.8), transparent); position: relative;
}
.control-btn.snap {
  width: 72px; height: 72px; border-radius: 50%; background: white; border: 4px solid rgba(255,255,255,0.3);
}
.control-btn.cancel {
  position: absolute; left: 30px; background: rgba(255,255,255,0.2); color: white;
  width: 40px; height: 40px; border-radius: 50%; font-size: 1.2rem; border: none;
}
</style>